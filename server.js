...
const AI_PROMPTS = {
  ...
  'template-15': { // Stock Broker / Demat (SEBI · NSE · BSE · MCX · CDSL / NSDL)
    hero:        ({biz,desc,tone}) => `For SEBI-registered stock broker "${biz}" (${tone} tone): "${desc}". Return ONLY JSON: { "heroEyebrow":"<trust pill max 10 words e.g. Trusted by 5 Lakh+ active investors>", "heroHeadlineLead":"<2-3 word line 1 e.g. Invest in>", "heroHeadlineEmph":"<1-3 word accent ending with period e.g. everything.>", "heroSub":"<30-45 word sub mentioning stocks, F&O, mutual funds, IPOs and SEBI registration>" }`,
    products:    ({biz,desc})      => `For SEBI-registered stock broker "${biz}": "${desc}". Return ONLY JSON: { "productsHeadline":"<8-12 word section headline>", "productsBody":"<25-35 word sub-copy>", "products":[{"icon":"<1 emoji from 📈 📊 🪙 🚀 🛡 💼 💰 🛢>","name":"<2-3 word product e.g. Stocks>","tagline":"<5-9 word tagline>","body":"<15-25 word description focused on investor outcome>"}] } with 6-8 products covering Stocks, F&O, Mutual Funds, IPO, ETFs, Bonds, MTF, Commodities.`,
    whyChoose:   ({biz,desc})      => `For SEBI-registered stock broker "${biz}": "${desc}". Return ONLY JSON: { "whyChooseHeadline":"<8-12 word section headline>", "whyChooseBody":"<25-35 word sub-copy>", "whyChoosePoints":[{"icon":"<1 emoji>","title":"<2-4 word title>","body":"<15-25 word benefit>"}] } with EXACTLY 6 points covering speed, security, zero MF commission, pro charts, mobile-first, 24x7 support.`,
    process:     ({biz,desc,tone}) => `For SEBI-registered stock broker "${biz}" (${tone}): "${desc}". Return ONLY JSON: { "processHeadline":"<8-12 word headline e.g. Open your Demat in minutes — fully digital.>", "processSteps":[{"icon":"<1 emoji>","title":"<2-3 word step>","body":"<15-25 word body>","duration":"<short e.g. 2 minutes>"}] } with EXACTLY 4 steps: sign up → Aadhaar eKYC → eSign with PAN → start investing.`,
  ...
  'template-18': { // Diagnostic Lab / Pathology (NABL + CAP + ICMR + CEA + BMW + PCPNDT)
    ...
    popularTests:  ({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "popularTestsHeadline":"<8-12 word section headline>", "popularTestsBody":"<25-35 word sub-copy>" } — produce only headline and body; the test cards are user-filled.`,
    homeCollection:({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "homeCollectionHeadline":"<8-14 word headline about the home collection promise>", "homeCollectionBody":"<40-70 word body about phlebotomist training, sample chain, slot windows, no minimum order>", "homeCollectionPerks":[{"icon":"<1 emoji from 🚚 ⏱ 🧪 🎓 🥶 📲>","label":"<10-16 word perk>"}] } with EXACTLY 6 perks covering free pickup, slot windows, sealed kits, trained staff, cold chain, SMS+WhatsApp tracking.`,
    process:       ({biz,desc,tone}) => `For NABL-accredited diagnostic lab "${biz}" (${tone}): "${desc}". Return ONLY JSON: { "processHeadline":"<8-12 word headline e.g. A test that just works.>", "processSteps":[{"icon":"<1 emoji>","title":"<2-3 word step>","body":"<15-25 word body>","duration":"<short e.g. 5 minutes>"}] } with EXACTLY 4 steps: book → sample collected → lab processing → digital report.`,
    packages:      ({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "packagesHeadline":"<8-12 word section headline>", "packagesBody":"<25-40 word sub-copy about how packages are curated>" } — only headline and body, packages are user-filled.`,
    reports:       ({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "reportsHeadline":"<8-12 word section headline>", "reportsBody":"<35-60 word sub-copy about report clarity, colour coding, pathologist commentary>", "reportFeatures":[{"icon":"<1 emoji>","title":"<2-4 word title>","body":"<10-20 word body>"}] } with EXACTLY 6 features covering colour coding, pathologist commentary, longitudinal tracking, multi-channel delivery, doctor share, data protection.`,
    specialists:   ({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "specialistsHeadline":"<8-12 word section headline>", "specialistsBody":"<25-35 word sub-copy about the team behind each report>" } — only headline and body.`,
    cta:           ({biz,desc})      => `For NABL-accredited diagnostic lab "${biz}": "${desc}". Return ONLY JSON: { "ctaHeadline":"<6-10 word headline about booking a test or home collection>", "ctaBody":"<20-30 word supporting line>", "ctaButton":"<2-4 word action e.g. Book Home Collection>", "ctaNote":"<short reassurance e.g. 24×7 Helpline · NABL accredited>" }`
  },
  'template-17': { // Healthcare Clinic / Hospital (NMC + NABH + Clinical Establishment Act)
    hero:         ({biz,desc,tone}) => `For NABH-accredited hospital / clinic "${biz}" (${tone} tone — warm and reassuring, NOT corporate): "${desc}". Return ONLY JSON: { "heroEyebrow":"<trust pill max 12 words e.g. NABH Accredited · 24/7 Emergency · Since 2008>", "heroHeadlineLead":"<2-4 word line 1 e.g. Healthcare that>", "heroHeadlineEmph":"<1-3 word accent ending with period e.g. listens.>", "heroSub":"<30-45 word sub about specialties, patient-first care, and the calm waiting-room feel>" }`,
    specialties:  ({biz,desc})      => `For NABH-accredited hospital "${biz}": "${desc}". Return ONLY JSON: { "specialtiesHeadline":"<8-12 word section headline>", "specialtiesBody":"<25-35 word sub-copy>", "specialties":[{"icon":"<1 emoji from ❤️ 🧠 🦴 👶 🤰 👁 🦷 🫁 🧬 🫀 🩺 🧴>","name":"<1-2 word specialty e.g. Cardiology>","body":"<12-20 word one-line scope>"}] } with 8-12 specialties covering common departments.`,
    doctors:      ({biz,desc})      => `For NABH-accredited hospital "${biz}": "${desc}". Return ONLY JSON: { "doctorsHeadline":"<8-12 word section headline>", "doctorsBody":"<25-35 word sub-copy about consultant continuity and trust>" } — produce only the headline and body, the doctor list itself is filled by the user.`,
    services:     ({biz,desc})      => `For NABH-accredited hospital "${biz}": "${desc}". Return ONLY JSON: { "servicesHeadline":"<8-12 word section headline>", "servicesBody":"<25-35 word sub-copy>", "services":[{"icon":"<1 emoji>","name":"<2-4 word service>","body":"<20-30 word description>"}] } with 6 services covering OPD, In-patient & ICU, 24/7 Emergency, Diagnostics, Pharmacy, Day-care Procedures.`,
    process:      ({biz,desc,tone}) => `For NABH-accredited hospital "${biz}" (${tone}): "${desc}". Return ONLY JSON: { "processHeadline":"<8-12 word headline e.g. How an appointment works here.>", "processSteps":[{"icon":"<1 emoji>","title":"<2-3 word step>","body":"<15-25 word body>","duration":"<short e.g. 20 minutes>"}] } with EXACTLY 4 steps: book → walk in → consult → leave with a written care plan.`,
    packages:     ({biz,desc})      => `For NABH-accredited hospital "${biz}": "${desc}". Return ONLY JSON: { "packagesHeadline":"<8-12 word section headline>", "packagesBody":"<25-35 word sub-copy>" } — produce only the headline and body for the preventive health-check packages section.`,
    about:        ({biz,desc,tone}) => `For NABH-accredited hospital "${biz}" (${tone}): "${desc}". Return ONLY JSON: { "aboutHeadlineLead":"<3-5 word line 1 e.g. A hospital you can>", "aboutHeadlineEmph":"<3-5 word accent ending with period e.g. walk into without dread.>", "aboutBody":"<120-180 word origin story — when founded, doctor-count, beds, NABH accreditation, what makes the place culturally different>", "aboutPillars":[{"icon":"<1 emoji>","title":"<2-3 word pillar>","body":"<20-30 word body>"}] } with EXACTLY 4 pillars covering patient-first, evidence-based, transparent pricing, accessibility/cashless.`,
    cta:          ({biz,desc})      => `For NABH-accredited hospital "${biz}": "${desc}". Return ONLY JSON: { "ctaHeadline":"<8-12 word headline about booking on time and being seen on time>", "ctaBody":"<20-30 word supporting line about walk-ins and the emergency helpline>", "ctaButton":"<2-3 word action e.g. Book Appointment>", "ctaNote":"<short reassurance e.g. 24/7 Emergency · NABH Accredited>" }`
  },
  'template-16': { // SEBI RIA / Investment Adviser (fee-only, fiduciary, warm/casual tone)
    hero:     ({biz,desc,tone}) => `For SEBI-registered Investment Adviser (RIA) "${biz}" (${tone} tone — warm and conversational, NOT corporate): "${desc}". Return ONLY JSON: { "heroEyebrow":"<trust pill max 12 words e.g. SEBI Registered Investment Adviser · Fee-only>", "heroHeadlineLead":"<3-5 word line 1 e.g. Your money decisions,>", "heroHeadlineEmph":"<2-5 word italic accent ending with period e.g. finally uncomplicated.>", "heroSub":"<30-45 word sub about fee-only model, no commissions, written plans, quarterly reviews — feel warm not corporate>" }`,
    fees:     ({biz,desc})      => `For SEBI-registered RIA "${biz}": "${desc}". Return ONLY JSON: { "feesHeadline":"<7-12 word headline e.g. Fee-only. No commissions. Ever.>", "feesBody":"<30-50 word sub-copy explaining the fee-only stance>", "feePlans":[{"name":"<2-4 word plan name>","price":"<headline price e.g. ₹60,000 / year>","priceNote":"<short caveat e.g. Per family · Paid quarterly>","ideal":"<who it suits in 8-12 words>","perks":[{"label":"<one inclusion line, max 16 words>"}]}] } with EXACTLY 2 plans — a Flat Fee Plan and an AUA-Linked Plan, each with 4-6 perks. Respect the SEBI fee cap: max 2.5% AUA OR ₹1,25,000 fixed per family per annum.`,
    approach: ({biz,desc})      => `For SEBI-registered RIA "${biz}": "${desc}". Return ONLY JSON: { "approachHeadline":"<8-12 word headline that feels human, not corporate>", "approachBody":"<25-40 word sub-copy>", "aboutPillars":[{"icon":"<1 emoji from 🤝 🪶 📚 🕯 🌱 🧭>","title":"<2-4 word title>","body":"<25-35 word body in warm conversational tone>"}] } with EXACTLY 4 pillars covering fiduciary duty, fee-only model, evidence-based planning, and calm/patient approach. AVOID corporate jargon.`,
    services: ({biz,desc})      => `For SEBI-registered RIA "${biz}": "${desc}". Return ONLY JSON: { "servicesHeadline":"<7-12 word headline e.g. A full financial life — handled together.>", "servicesBody":"<25-40 word sub-copy>", "services":[{"icon":"<1 emoji>","name":"<2-3 word service e.g. Goal Planning>","body":"<20-30 word warm description>"}] } with 5-6 services covering Financial Planning, Goal Planning, Investment Advisory, Tax Planning, Estate Planning Review, Insurance Review. Remember: RIAs DO NOT sell insurance — they only review existing covers.`,
    process:  ({biz,desc,tone}) => `For SEBI-registered RIA "${biz}" (${tone} — warm and conversational): "${desc}". Return ONLY JSON: { "processHeadline":"<8-12 word headline e.g. From first call to first review — without the pitch.>", "processSteps":[{"icon":"<1 emoji>","title":"<2-3 word step e.g. Discovery call>","body":"<20-30 word warm body>","duration":"<short e.g. 30 minutes>"}] } with EXACTLY 4 steps: discovery call → written plan → implementation → quarterly catch-ups.`,
    about:    ({biz,desc,tone}) => `For SEBI-registered RIA "${biz}" (${tone}): "${desc}". Return ONLY JSON: { "aboutHeadlineLead":"<3-5 word line 1 e.g. A small practice>", "aboutHeadlineEmph":"<2-3 word italic accent ending with period e.g. by design.>", "adviserBio":"<140-180 word origin story — why you left commission-based wealth management, when you registered with SEBI, why the practice is intentionally small, what you believe about advice. Conversational and personal. Avoid corporate boilerplate.>" }`,
    articles: ({biz,desc})      => `For SEBI-registered RIA "${biz}": "${desc}". Return ONLY JSON: { "articlesHeadline":"<7-12 word section headline>", "articlesBody":"<25-40 word sub-copy>", "articles":[{"category":"<1-2 word category e.g. Planning, Investing, Tax, Retirement>","title":"<6-12 word article title in plain English>","body":"<20-30 word snippet>","readTime":"<short e.g. 6 min read>"}] } with EXACTLY 4 articles covering financial-planning basics, why index funds, tax for salaried, and retirement framework.`,
    cta:      ({biz,desc})      => `For SEBI-registered RIA "${biz}": "${desc}". Return ONLY JSON: { "ctaHeadline":"<8-12 word warm headline about booking a free intro call — NOT urgent or salesy>", "ctaBody":"<20-30 word supporting line about no-pitch, no-obligation 30-minute chat>", "ctaButton":"<2-5 word action e.g. Book a free intro call>", "ctaNote":"<short reassurance e.g. Fee-only · SEBI Registered>" }`
  },
...
const TEMPLATE_NAMES = {
  ...
  'template-15': 'Stock Broker / Demat',
